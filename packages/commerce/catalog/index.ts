import "server-only";
import { getProductPresentation } from "../../../app/lib/product-presentation";
...
type CatalogApiProduct = Omit<
  CatalogProduct,
  "image" | "price" | "compareAtPrice" | "collectionTitle" | "supplierReference"
>;
...
export function normalizeProduct(product: CatalogApiProduct): CatalogProduct {
  ...
  return {
    ...
    compareAtPrice:
      lowest?.compareAtPrice && lowest.compareAtPrice > lowest.price ? lowest.compareAtPrice : undefined,
    collectionTitle: product.tags[0] || "Slow Fit",
    supplierReference: presentation.supplierReference,
  };
}
...
export async function getProductPage({ page = 1, pageSize = 24, search = "", tag = "" } = {}): Promise<CatalogPage> {
  ...
  const payload = await catalogFetch<{ products: CatalogApiProduct[]; total: number; page: number; pageSize: number }>(
...
export async function getProductByHandle(handle: string): Promise<CatalogProduct | null> {
  const payload = await catalogFetch<{ product: CatalogApiProduct }>(`/api/catalog/products/${encodeURIComponent(handle)}`);
